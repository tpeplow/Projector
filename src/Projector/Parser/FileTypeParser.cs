...
namespace Projector.Parser
{
    ...
    {
        static readonly Regex FileTypeRegex = new Regex("([\\*\\./a-z0-9]+)\\s*:((?:\\s*[a-z]+\\s*=\\s*[a-z]+;)+)", RegexOptions.IgnoreCase);
        static readonly Regex FileTypeOptionsRegex = new Regex("\\s*([a-z]+)\\s*=\\s*([a-z]+);", RegexOptions.IgnoreCase);
